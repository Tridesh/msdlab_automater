...
"""
ENHANCED TCAD TCL Command Generator – Multi-Mode Version
...
# ──────────────────────────────────────────────
# Mode 1: Many nodes with user TDRs and optional last TDR
# ──────────────────────────────────────────────
...
def generate_mode1_commands():
    ...
    try:
        ...
        for i in range(no_of_tdrs):
            code = input(f"Enter TDR-{i+1} code (e.g., 0002): ")
            tdr_codes.append(code)
        ...
        while True:
            ...
            parameter = input("Enter the parameter for this set (as shown above): ")
            while parameter not in parameters:
                print("Invalid parameter. Please choose from the available list.")
                parameter = input("Enter the parameter for this set (as shown above): ")
        ...
        for node in node_names:
            for code in tdr_codes:
                all_files.append(f"{node}_{code}")
            if need_last_tdr in ['yes', 'y']:
                last_prefix = node.split('_')[0]
                all_files.append(f"{last_prefix}")
        ...
        first_plot = f"Plot_{all_files[0]}_des"
        ...
        for set_idx, cutset in enumerate(cutplane_cutline_sets, start=1):
            ...
            parameter = cutset['parameter']
            axis_x = get_axis_mapping(cp_axis, cl_axis)

            last_plot = f"Plot_{all_files[-1]}_des"
            last_dset = f"{all_files[-1]}_des"
            cmds += [
            ...
            for i, file_base in enumerate(all_files):
            ...
            if set_idx == 1:
                unified_1d_plot = f"Plot_C1(C1({all_files[0]}_des))"
                first_cl_dset = f"C1(C1({all_files[0]}_des))"
                cmds += [
                ...
                for i, file_base in enumerate(all_files):
                    cl_dset = f"C1(C1({file_base}_des))"
                    cmds += [
        ...
        all_curve_names = " ".join(f"Curve_{i}" for i in range(1, curve_counter))
        csv_full_path = f"{csv_export_path}{csv_filename}.csv"
        cmds += [
            f"select_plots {{{unified_1d_plot}}}",
            f"# {unified_1d_plot}",
            f"export_curves {{{all_curve_names}}} -plot {unified_1d_plot} -filename {csv_full_path} -format csv",
            f"# {csv_full_path}"
        ]
...
# ──────────────────────────────────────────────
# Mode 2: Different nodes with PLT files
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
# Mode 3: Multiple Cutplane-Cutline-Parameter Sets (Dynamic, Fixed)
# ──────────────────────────────────────────────
...
def generate_mode3_commands():
    ...
    try:
        ...
        tdr_codes = []
        for i in range(no_of_tdrs):
            code = input(f"Enter TDR-{i + 1} code (e.g., 0000): ")
            tdr_codes.append(code)
        cutplane_cutline_sets = []
        while True:
            print("\n--- New Set ---")
        ...
        for set_idx, cutset in enumerate(cutplane_cutline_sets, start=1):
            ...
            cmds += [
                ...
                "################################################",
                f"# --- Set {set_idx}: {parameter} ---"
            ]
            # Load all TDRs and create plots
            for tdr_idx, code in enumerate(tdr_codes):
                fname = f"{file_prefix}{code}_des.tdr"
                dset = f"{file_prefix}{code}_des"
                plot = f"Plot_{dset}"
                cmds += [
                    f"load_file {base_path}{fname} -fod",
                    f"create_plot -dataset {dset}",
            ...
            # Link all plots
            first_plot = f"Plot_{file_prefix}{tdr_codes[0]}_des"
            all_plots = " ".join(f"Plot_{file_prefix}{code}_des" for code in tdr_codes)
            cmds += [
                f"select_plots {{{first_plot}}}",
                f"# {first_plot}",
                f"select_plots {{{all_plots}}}",
            ...
            last_plot = f"Plot_{file_prefix}{tdr_codes[-1]}_des"
            last_dset = f"{file_prefix}{tdr_codes[-1]}_des"
            cmds += [
                f"set_field_prop {parameter} -plot {last_plot} -geom {last_dset} -show_bands",
                "# 0"
            ]
            ...
            for code in tdr_codes:
                plot = f"Plot_{file_prefix}{code}_des"
                cmds += [
                    f"select_plots {{{plot}}}",
                    f"# {plot}",
                    f"create_cutplane -plot {plot} -type {cp_axis} -at {cp_pos}",
                    f"# C{set_idx}({file_prefix}{code}_des)"
                ]
            # Create plots for cutplanes
            for code in tdr_codes:
                cp_dset = f"C{set_idx}({file_prefix}{code}_des)"
                plot = f"Plot_{file_prefix}{code}_des"
                cmds += [
                    f"create_plot -dataset {cp_dset} -ref_plot {plot}",
                    f"select_plots {{Plot_{cp_dset}}}",
            ...
            # Create cutlines and 1D plot for this set
            for tdr_idx, code in enumerate(tdr_codes):
                cp_plot = f"Plot_C{set_idx}({file_prefix}{code}_des)"
                cl_pos = cl_positions[tdr_idx]
                cmds += [
                    f"select_plots {{{cp_plot}}}",
                    f"# {cp_plot}",
                    f"create_cutline -plot {cp_plot} -type {cl_axis} -at {cl_pos}",
                    f"# C1(C{set_idx}({file_prefix}{code}_des))"
                ]
            # Create 1D plot for this set (each set gets its own 1D plot, matching example)
            plot1d_name = f"Plot_C1(C{set_idx}({file_prefix}{tdr_codes[0]}_des))"
            plot_names.append(plot1d_name)
            first_cl_dset = f"C1(C{set_idx}({file_prefix}{tdr_codes[0]}_des))"
            cmds += [
                f"create_plot -dataset {first_cl_dset} -1d",
                f"select_plots {{{plot1d_name}}}",
                f"# {plot1d_name}", "", f"# {plot1d_name}"
            ]
            # Add curves for each TDR in this set
            for tdr_idx, code in enumerate(tdr_codes):
                cl_dset = f"C1(C{set_idx}({file_prefix}{code}_des))"
                cmds += [
            ...
            # Export curves to CSV for this set
            all_curve_names = " ".join(f"Curve_{i+1}" for i in range(len(tdr_codes)))
            csv_full_path = f"{csv_export_path}{csv_filename}_set{set_idx}.csv"
            cmds += [
                f"select_plots {{{plot1d_name}}}",
                f"# {plot1d_name}",
...
def run_single_workflow():
    print("=" * 80)
    print("ENHANCED TCAD TCL COMMAND GENERATOR – Multi-Mode")
    print("=" * 80)
...
def main():
    print("=" * 80)
    print("WELCOME TO TCAD TCL COMMAND GENERATOR")
    print("=" * 80)
    while True:
        try:
